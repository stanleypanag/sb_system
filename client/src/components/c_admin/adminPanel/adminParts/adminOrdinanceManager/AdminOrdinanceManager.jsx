...
const AdminOrdinanceManager = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetch("http://192.168.1.10:5000/api/documents/ordinance/").then((res) =>
        res.json()
      ),
  });
  ...
  const editOrdinanceMutation = useMutation({
    mutationFn: (formData) =>
      fetch(
        `http://192.168.1.10:5000/api/documents/${editingOrdinance.doc_id}`,
        {
          method: "PUT",
          body: formData,
        }
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {ordinances.data.map((ordinance) => (
                      <tr key={ordinance.doc_id}>
                        ...
                        <td className="py-4 whitespace-nowrap text-sm text-gray-800 text-wrap text-justify">
                        ...
                        <td className="px-6 flex gap-3 whitespace-nowrap justify-end">
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleOpenModal(
                                editOrdinanceMutation.mutate(ordinance)
                              )
                            }
                          >
                            Edit
